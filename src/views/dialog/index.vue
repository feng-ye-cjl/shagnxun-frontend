<template>
  <div class="main">
    <div class="select">
      <!--公司id-->
      <el-input class="in" v-model="companyId" placeholder="公司id" style="width: 100px; margin-right: 50px" @keydown.enter="getRouter"/>
      <!--   根据id查询公司实体列表按钮     -->
    ...
    <div class="companyInfo">
      ...
      <!--   新增按钮     -->
      <el-button type="primary" @click="addBusEvent">新增</el-button>
      <el-button type="primary" @click="testAdd">测试</el-button>
    </div>
    <!--公司主体和子主体表格-->
...
const testAdd = async () => {
  const requestBody = {
    ...
    eventCode2 : "002", 
    eventName2 : "施工",
    property_english : "entityNo",
    property_chinese : "主体编号",
    property_english2 : "filed9",
    property_chinese2 : "关联编号",
    Role_type : "00",
  }
  const res = await company2Api.BusEntityEventServletFloating(requestBody)
  console.log('busEvent响应结果 = >', res)
};

// 新增函数
const addBusEvent = async () => {
  ...
  const requestBody = {
    ...
    entityName: entityInfo.value.codeName,
    entityType: entityInfo.value.entityType,
    typeName: entityInfo.value.typeName,
    eventCode: entityInfo.value.eventCode,
    eventName: entityInfo.value.eventName,
    ...
    entityName2: entityInfo2.value.codeName,
